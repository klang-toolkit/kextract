...
The kextract tool parses header (.h) files of native libraries, and generates Java code,
...
the structs they want to access. The kextract tool aims to automate many of these steps,
so that a client can instead immediately start using the native libraries they are
interested in.

This guide shows how to run the kextract tool, and how to use the Java code that it generates.
The samples under the [`samples`](../samples) directory are also a good source of examples.

Note that at this time, kextract (and FFM) only supports C header files. If you have a
...
directory corresponding to the root package. A typical way to run kextract would be like
this:
...
$ kextract \
  ...
  --output src \
  --target-package org.kextract.mylib \
  --library mylib \
...
- `--target-package org.kextract.mylib` specifies the target package to which the generated
  classes and interfaces will belong. (Note that kextract will automatically create the
  directories representing the package structure under the `src` directory specified
  through `--output`)
- `--library mylib` tells kextract that the generated bindings should load the library
  called `mylib`. (The section on [library loading](#library-loading) discusses how this
  is done)

Note that specifying the wrong header file to kextract may result in errors during parsing.
Please consult the documentation of the library in question about which header file
should be included. This is also the header file that should be passed to kextract.

If a library has multiple main header files, they can be passed to kextract on the command line.
...
Besides these options, it is also possible to filter the output of kextract using one of the `--include-XYZ` options
that kextract has. See the section on [filtering](#filtering) for a more detailed overview. See also the full
list of command line options [here](#command-line-option-reference).

Most of the code that kextract generates will be available through a single class. By default,
the name of that class is derived from the name of the main header file that is passed to
kextract. For example, if the header file is named `mylib.h`, then the derived class name
will be `mylib_h`. The class name can be set explicitly using the `--header-class-name`
option as well.

Besides the main header class that is generated, kextract also generates several
...
[Using The Code Generated By Kextract section](#using-the-code-generated-by-kextract)).

Generally speaking, the bindings generated by kextract depend on the platform on which
kextract is running. For example, when a C header file is processed by the C pre-processor,
...
examples of things that can lead kextract to generate different outputs depending on the
platform on which it is run. Care should be taken when generating bindings with kextract
...
Kextract assumes that the version of a native library that a project uses is relatively stable.
Therefore, kextract is intended to be run once, and then for the generated sources to be added to the project.
Kextract only needs to be run again when the native library, or kextract itself are updated.
(This is also the workflow that kextract itself follows: kextract depends on the
[libclang](https://clang.llvm.org/docs/LibClang.html) native library in order to parse C sources).

## Using The Code Generated By Kextract

In the following section we'll go over examples of declarations that might be found in a C
header file, show the code that kextract generates for these declarations, and show
some examples of how to use the generated Java code.

Most of the methods that kextract generates are `static`, and are designed to be imported
using `import static`. Typically, to access the code that kextract generates for a header
...
Where `org.mypackage` is the package into which kextract puts the generates source files
(using `--target-package`/`-t`).

The former import statement will import all the static functions and fields from the class
that kextract generates for the main header file of the library. This includes methods to
access functions, global variables, macros, enums, primitive typedefs, and layouts for
builtin C types.

The latter import statement imports all the other classes generated by kextract, which
include: classes representing structs or unions, function types, and struct or union
typedefs.

### Builtin Type Layouts

For every kextract run, regardless of the contents of the library header files, kextract
...
`T*`). Note that the layouts that kextract generates depend on the platform that kextract
...
Kextract will generate the following set of methods for this function in the main header
class it generates:
...
Kextract generates the following:
...
Besides that, kextract also generates a getter and a setter method to get and set the value
...
Kextract will generate simple getter methods to access the constant values of the macro
...
not supported by kextract. (See the section on [unsupported features](#unsupported-features))

Note that for macros, kextract only generates an accessor when it sees a macro definition,
...
Kextract generates a separate class which roughly looks like the following:
...
The class that kextract generates for unions is identical to the class generated for
structs.

### Function Pointers

Kextract generates a separate class for each function pointer type found in the header
...
Kextract generates the following class in a `callback_t.java` file:
...
through the header files passed to kextract):
...
returns in Java using the `invoke` method in the `callback_t` class that kextract generates
for us:
...
Kextract generates function pointer classes like the `callback_t` class for function
...
Kextract handles variadic functions differently from regular functions. Variadic functions
...
To make calling variadic functions easier, kextract introduces the concept of an _invoker_.
...
Kextract doesn't generate a regular method, but a _class_, which represents the invoker:
...
in the main header class that kextract generates, or as a separate class, depending on
...
`MyInt` is a `typedef` of the primitive type `int`, so it is translated by kextract as a
...
Kextract treats variables (global variables or struct/union fields) with an array type
...
Kextract generates a few extra methods that are useful for working with arrays:
...
1. a getter and setter pair for the array variable. Note that the getter replaces the usual
  `XYZ$segment` method that kextract generates for global variables, as the results of the
...
Kextract generates a _nested_ struct and function pointer class for the `bar` and `cb`
...
Finally, there are some features that kextract does not support, listed below:
...
- Opaque types. When a struct or union type is declared but not defined, like:
  ...
  Kextract is not able to generate the regular `Foo` class. You will see a warning about
...
- Linker options. It is currently not possible to tell kextract to use linker options to
...
The value of these macros also affects the behavior of kextract. Therefore, kextract
...
Please note that other header files included by kextract may also define macro values using
the `#define` pre-processor directive. It is therefore important to notice the order in
which header files are processed by a compiler, as feeding header files to kextract in the
...
should be passed to kextract for things to work correctly. Please consult the
documentation of the library that you're trying to use to find out which header file should
be included/passed to kextract.
...
specified to kextract, the generated bindings will try to load libraries specified using
...
interested in letting kextract generate code for the entire library. In cases like that,
we can use kextract's `--include-XXX` command line options to only generate classes for
the elements we specify.

To allow for symbol filtering, kextract can generate a _dump_ of all the symbols
...
bindings only for a _subset_ of symbols seen by kextract. For instance, if we run
kextract with as follows:

```sh
$ kextract --dump-includes includes.txt mylib.h
```
...
and passed back to kextract:

```sh
$ kextract --dump-includes includes.txt mylib.h
$ grep Foo includes.txt > includes_filtered.txt
$ kextract @includes_filtered.txt mylib.h
```
...
Here, we could run kextract and filter out `A`, like so:

```sh
$ kextract --include-var aVar test.h
```
...
In such cases, kextract will report the missing dependency and terminate without
...
diagnosing application bugs and/or crashes. The code generated by the kextract tool
supports _tracing_ of native calls, that is, parameters passed to native calls can be
printed on the standard output.

To enable the tracing support, just pass the `-Dkextract.trace.downcalls=true` flag as a VM
...
Kextract accepts one or more header files. When multiple header files are specified,
...
Kextract uses an embedded clang compiler (through libclang) to parse header files. Users
...
As noted in the introduction, kextract currently only supports C header files, but many
other languages also support C interop, and kextract/FFM can still be used to talk to
...
| :---------| ------------------------------------------------------------ |
| C++       | C++ allows declaring C methods using `extern "C"`, and many C++ libraries have a C interface to go with them. Kextract can consume such a C interface, which can then be used to access the library in question. |
| Rust      | The Rust ecosystem has a tool called `cbindgen` which can be used to generate a C interface for a Rust library. Such a generated C interface can then be consumed by kextract, and be used to access the library in question. |
