...
class MissingDepChecker(private val logger: Logger) : Declaration.Visitor<Void?, Declaration?> {
    ...
    override fun visitFunction(funcTree: Declaration.Function, parent: Declaration?): Void? {
        ...
        val posDecl = posDecl(funcTree, parent)
        funcTree.parameters().forEach { it.accept(this, posDecl) }
        Utils.forEachNested(funcTree) { it.accept(this, posDecl) }
    ...
    override fun visitScoped(d: Declaration.Scoped, parent: Declaration?): Void? {
        ...
        val posDecl = posDecl(d, parent)
        d.members().forEach { it.accept(this, posDecl) }
        return null
    }
    ...
    override fun visitTypedef(tree: Declaration.Typedef, parent: Declaration?): Void? {
        ...
        val posDecl = posDecl(tree, parent)
        Utils.forEachNested(tree) { it.accept(this, posDecl) }
    ...
    private fun posDecl(decl: Declaration, parent: Declaration?): Declaration = parent ?: decl
