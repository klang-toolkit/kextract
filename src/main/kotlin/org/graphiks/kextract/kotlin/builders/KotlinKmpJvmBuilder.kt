...
internal class KotlinKmpJvmBuilder(
     ...
     *   `callStructArgWGPUStringViewRetP(fn, structPtr): Long` (wgpuGetProcAddress —
     *   nom dédié au retour pointeur, pas d'overload sur la forme Unit).
     *
     ...
     * de JvmDowncallEngine : nom de forme canonique → (forme `&lt;argLetters&gt;|&lt;returnKind&gt;`
     * → nom réel du wrapper émis). argLetters encode chaque paramètre
     * (I/L/P/F/D/S/B comme les wrappers scalaires, S = struct par valeur à sa
     * position C) et returnKind vaut V (Unit), P (pointeur, résultat Long du
     * moteur) ou S (struct par valeur, ByValue). La valeur du mapping est le nom
     * exact de la méthode du moteur — les formes dont le retour diverge de la forme
     * canonique portent un nom dédié (callStructArgWGPUStringViewRetP pour le
     * retour pointeur, jamais d'overload). Toute signature hors table échoue à la
     * génération plutôt que d'émettre un appel irrésolu (arity/type) qui ne
     * compilerait pas.
     */
    private val jvmEngineStructWrapperShapes: Map<String, Map<String, String>> = mapOf(
        "callStructArgBox" to mapOf("S|V" to "callStructArgBox"),
        "callStructArgWGPUStringView" to mapOf(
            "S|P" to "callStructArgWGPUStringViewRetP",
            "PS|V" to "callStructArgWGPUStringView",
        ),
        "callStructArgWGPUAdapterInfo" to mapOf("S|V" to "callStructArgWGPUAdapterInfo"),
        "callStructArgWGPUSupportedFeatures" to mapOf("S|V" to "callStructArgWGPUSupportedFeatures"),
        "callStructArgWGPUSupportedInstanceFeatures" to mapOf("S|V" to "callStructArgWGPUSupportedInstanceFeatures"),
        "callStructArgWGPUSupportedWGSLLanguageFeatures" to mapOf("S|V" to "callStructArgWGPUSupportedWGSLLanguageFeatures"),
        "callStructArgWGPUSurfaceCapabilities" to mapOf("S|V" to "callStructArgWGPUSurfaceCapabilities"),
        "callStructReturnBox" to mapOf("I|S" to "callStructReturnBox"),
        "callStructReturnWGPUFuture" to mapOf("P|S" to "callStructReturnWGPUFuture"),
        "callStructReturnWGPUFutureWGPUQueueWorkDoneCallbackInfo" to mapOf("PS|S" to "callStructReturnWGPUFutureWGPUQueueWorkDoneCallbackInfo"),
        "callStructReturnWGPUFutureWGPUPopErrorScopeCallbackInfo" to mapOf("PS|S" to "callStructReturnWGPUFutureWGPUPopErrorScopeCallbackInfo"),
        "callStructReturnWGPUFutureWGPUCompilationInfoCallbackInfo" to mapOf("PS|S" to "callStructReturnWGPUFutureWGPUCompilationInfoCallbackInfo"),
        "callStructReturnWGPUFutureWGPURequestAdapterCallbackInfo" to mapOf("PPS|S" to "callStructReturnWGPUFutureWGPURequestAdapterCallbackInfo"),
        "callStructReturnWGPUFutureWGPURequestDeviceCallbackInfo" to mapOf("PPS|S" to "callStructReturnWGPUFutureWGPURequestDeviceCallbackInfo"),
        "callStructReturnWGPUFutureWGPUCreateRenderPipelineAsyncCallbackInfo" to mapOf("PPS|S" to "callStructReturnWGPUFutureWGPUCreateRenderPipelineAsyncCallbackInfo"),
        "callStructReturnWGPUFutureWGPUCreateComputePipelineAsyncCallbackInfo" to mapOf("PPS|S" to "callStructReturnWGPUFutureWGPUCreateComputePipelineAsyncCallbackInfo"),
        "callStructReturnWGPUFutureWGPUBufferMapCallbackInfo" to mapOf("PLLLS|S" to "callStructReturnWGPUFutureWGPUBufferMapCallbackInfo"),
    )
    ...
    private fun emitStructByValueFunction(
        ...
        val emittedWrapper = jvmEngineStructWrapperShapes[wrapper]?.get("$argLetters|$returnKind")
        if (emittedWrapper == null) {
            emitGenericFunction(decl)
            return
        }
        val params = (
        ...
        builder.indent()
        val call = structByValueCall(decl, emittedWrapper, argStruct)
        when {
     ...
     * argument). [wrapper] est le nom EXACT de la méthode du moteur résolu par
     * [jvmEngineStructWrapperShapes] ; l'allocator n'est passé que pour les retours
     * struct (convention FFM), identifiés par le préfixe callStructReturn.
     */
    private fun structByValueCall(
        ...
        return "$jvmDowncallEngine.$wrapper(${functionAddress(decl)}" +
            (if (wrapper.startsWith("callStructReturn")) ", allocator" else "") +
    ...
    /**
     * Emits a downcall through the engine's canonical ABI model when no specialized
     * wrapper exists. This keeps generation total: a new C signature must not turn
     * into an uncompilable binding merely because the finite typed-wrapper table has
    ...
    private fun emitGenericFunction(decl: Declaration.Function) {
        ...
        val cName = decl.name()
        val returnType = typeMapper.mapFunctionType(decl.type().returnType())
        val returnsStruct = typeMapper.returnsStructByValue(decl.type().returnType())
        val params = (
            typeMapper.allocatorParams(decl.type().returnType()) +
                decl.parameters().map { param ->
                    "${namePlan.parameter(param)}: ${typeMapper.mapFunctionType(param.type())}"
        ...
        builder.appendLine("actual fun $name($params): $returnType {")
        builder.indent()
        emitGenericDowncall(decl, returnsStruct)
        builder.unindent()
        builder.appendLine("}")
        builder.appendLine()
    }
    ...
    private fun emitGenericDowncall(
        ...
        val resultType = genericAbiType(function.type().returnType(), isReturn = true)
        val argumentTypes = function.parameters()
            .joinToString(", ") { genericAbiType(it.type()) }
        val shape =
            "$jvmDowncallEngine.FunctionShape(result = $resultType, arguments = listOf($argumentTypes))"
        val arguments = function.parameters().map { parameter ->
            toRawJvmArgument(namePlan.parameter(parameter), parameter.type())
        }
        val call = "$jvmDowncallEngine.callGeneric(${functionAddress(function)}, $shape" +
            arguments.joinToString(separator = "", prefix = "", transform = { ", $it" }) + ")"
        val returnType = typeMapper.mapFunctionType(function.type().returnType())
        if (returnType == "Unit") {
            builder.appendLine(call)
            builder.appendLine("return")
            return
        ...
        val resultName = privateNames.allocate("__kffiGenericResult", "genericResult")
        builder.appendLine("val $resultName = $call")
        when (val abi = KotlinKmpCAbiType.from(function.type().returnType(), KotlinKmpAbiContext.DIRECT)) {
            is KotlinKmpCAbiType.StructValue -> {
                check(returnsStruct) { "generic struct result lost its struct marker" }
                usedSymbols += MEMORY_SEGMENT
                builder.appendLine(
                    "return $returnType.ByValue($nativeAddress(($resultName as $memorySegment).address()))",
                )
            }
            is KotlinKmpCAbiType.Address -> {
                ...
                emitEngineReturn(
                    ...
                    "($resultName as $memorySegment).address()",
                )
            }
            is KotlinKmpCAbiType.Scalar -> emitGenericScalarReturn(
                function.type().returnType(),
                abi,
                resultName,
            )
    ...
    private fun emitGenericScalarReturn(
        type: Type,
        abi: KotlinKmpCAbiType.Scalar,
        resultName: String,
    ) {
        val carrier = "($resultName as ${abi.jvmCarrier})"
        ...
        when {
            typeMapper.isOptionsEnumType(type) -> {
                ...
                builder.appendLine(
                    "return $returnType(${scalar.jvmCarrierToOptionsRaw(carrier)})",
                )
            }
            typeMapper.isEnumType(type) -> {
                val scalar = abiIndex.enum(typeMapper.enumDeclaration(type))
                builder.appendLine("return ${scalar.fromJvmCarrier(carrier)}")
            }
            abi.kind == KotlinKmpCAbiType.Scalar.Kind.BOOL ->
                builder.appendLine("return $carrier")
            else -> builder.appendLine("return ${abi.fromJvmCarrier(carrier)}")
        }
    }

    private fun genericAbiType(type: Type, isReturn: Boolean = false): String {
        if (isReturn && type is Type.Primitive && type.kind() == Type.Primitive.Kind.Void) {
            return "$jvmDowncallEngine.AbiType.Void"
        }
        return when (val abi = KotlinKmpCAbiType.from(type, KotlinKmpAbiContext.DIRECT)) {
            is KotlinKmpCAbiType.Address -> "$jvmDowncallEngine.AbiType.Pointer"
            is KotlinKmpCAbiType.StructValue ->
                "$jvmDowncallEngine.AbiType.Struct(\"${escapeKotlinString(namePlan.declaration(abi.declaration))}\")"
            is KotlinKmpCAbiType.Scalar -> {
                val member = when (abi.kind) {
                    KotlinKmpCAbiType.Scalar.Kind.BOOL -> "Bool"
                    KotlinKmpCAbiType.Scalar.Kind.I8 -> "I8"
                    KotlinKmpCAbiType.Scalar.Kind.I16 -> "I16"
                    KotlinKmpCAbiType.Scalar.Kind.CHAR16 -> "Char16"
                    KotlinKmpCAbiType.Scalar.Kind.I32 -> "I32"
                    KotlinKmpCAbiType.Scalar.Kind.I64 -> "I64"
                    KotlinKmpCAbiType.Scalar.Kind.F32 -> "F32"
                    KotlinKmpCAbiType.Scalar.Kind.F64 -> "F64"
                }
                "$jvmDowncallEngine.AbiType.$member"
            }
        }
    }
    ...
    private fun toRawJvmArgument(name: String, type: Type): String {
        ...
        return when {
            abi is KotlinKmpCAbiType.StructValue -> "$name.handler.rawValue"
            typeMapper.isOptionsEnumType(type) ->
