...
import org.openjdk.kextract.KextractTool;
...
public class KextractToolRunner {
    ...
    // (private) exit codes from kextract tool. Copied from KextractTool.
    ...
    private static final ToolProvider JEXTRACT_TOOL = new KextractTool.KextractToolProvider();

    private final Path inputDir;
    private final Path outputDir;

    protected KextractToolRunner() {
        this(null, null);
    }

    protected KextractToolRunner(Path input, Path output) {
    ...
    protected static class KextractResult {
        private int exitCode;
        private String output;

        KextractResult(int exitCode, String output) {
            this.exitCode = exitCode;
            this.output = output;
        }

        public KextractResult checkSuccess() {
            assertEquals(exitCode, SUCCESS, "Sucess expected, failed: " + exitCode + ", Output: " + output);
            return this;
        }

        public KextractResult checkFailure() {
            assertNotEquals(exitCode, SUCCESS, "Failure expected, succeeded!");
            return this;
        }

        public KextractResult checkFailure(int expectedExitCode) {
            assertEquals(exitCode, expectedExitCode, "Expected error code " + expectedExitCode);
            return this;
        }

        public KextractResult checkContainsOutput(String expected) {
            Objects.requireNonNull(expected);
            assertTrue(output.contains(expected), "Output does not contain string: " + expected);
            return this;
        }

        public KextractResult checkDoesNotContainOutput(String expected) {
            Objects.requireNonNull(expected);
            assertFalse(output.contains(expected), "Output contains string: " + expected);
            return this;
        }

        public KextractResult checkMatchesOutput(String regex) {
    ...
    protected static KextractResult runAndCompile(Path outputDir, Object... options) {
        return runAndCompile(outputDir, Arrays.stream(options).map(Objects::toString).toArray(String[]::new));
    }

    protected static KextractResult runAndCompile(Path outputDir, String... options) {
        KextractResult kextractResult = run(outputDir, options).checkSuccess();
        TestUtils.compile(outputDir, outputDir);
        return kextractResult;
    }

    protected static KextractResult run(Path outputDir, String... options) {
    ...
    protected static KextractResult runNoOuput(String... options) {
        StringWriter writer = new StringWriter();
        PrintWriter pw = new PrintWriter(writer);
        int result = JEXTRACT_TOOL.run(pw, pw, options);
        String output = writer.toString();
        System.err.println(output);
        return new KextractResult(result, output);
    }
    ...
    protected static Loader classLoader(Path... paths) {
        try {
            ...
            URLClassLoader ucl = new URLClassLoader(urls,
                    KextractToolRunner.class.getClassLoader());
